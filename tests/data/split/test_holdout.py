...
import mltools.data.split.holdout as mdpt
...
@pytest.mark.parametrize(
    ("val_size", "test_size", "val_exp", "test_exp"),
    [
        (0.2, 0.2, 8, 8),
        (0.25, 0.5, 10, 20),
        (5, 5, 5, 5),
...
@pytest.mark.parametrize(
    ("val_values", "test_values", "val_to_test_ratio", "expected"),
    [
        (["A"], ["B"], 0.5, (10, 10)),
        (["A", "B"], ["B", "D"], 0.5, (15, 15)),
        (["A", "B"], ["D"], 0.5, (20, 10)),
...
def test_compute_oos_tts_split(data, val_values, test_values, val_to_test_ratio, expected):
    val_exp, test_exp = expected
...
@pytest.mark.parametrize(
    ("val_values", "test_values", "val_to_test_ratio"),
    [
        (["A"], ["A"], 1.5),
        (["A", "B", "C", "D"], ["A", "B", "C", "D"], 0.5),
        (["A", "B", "C", "D"], [], 0.0),
...
def test_compute_oos_tts_split_invalid(data, val_values, test_values, val_to_test_ratio):
    with pytest.raises(ValueError, match=r"val_to_test_ratio|set is empty"):
